  ...
  upload(req, res, async (err) => {
    try {
      ...
      if (videos.length && !images.length && videos.length > 4) {
        return res.status(400).json({
          success: false,
          message: "You can upload up to 4 videos only"
        });
      }
      ...
      const [uploadedImages, uploadedVideos] = await Promise.all([
        Promise.all(images.map((img) =>
          uploadToCloudinary(img.buffer, "posts/images", "image")
        )),

        Promise.all(videos.map((vid) =>
          uploadToCloudinary(vid.buffer, "posts/videos", "video")
        )),
      ]);
